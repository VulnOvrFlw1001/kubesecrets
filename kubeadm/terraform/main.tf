...
resource "google_compute_instance" "instance" {
    ...
    inline = [ 
      "#!/bin/bash",
      "WEBHOOK_HOSTNAME=\"webhook-server\"",
      "if [ \"$(hostname)\" == \"$WEBHOOK_HOSTNAME\" ]; then",
      "echo 'export POSTGRES_PASSWORD=${random_password.password.result}' >> ~/.bashrc",
      "source ~/.bashrc",
      "fi"
     ]
    connection {
      type = "ssh"
      user = "hansj"
      host = google_compute_address.address[count.index].address
